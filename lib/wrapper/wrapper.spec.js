...
const URLS = {
  http: 'http://api.zippopotamus.us',
  https: 'https://api.zippopotamus.us'
};
...
describe('Wrapper', () => {
  describe('#query', () => {
    ...
    it('gets information for a place given the country, state and city', () => {
      nock(URLS.https)
        .get(`/${data.us_city_valid.request.country}/${data.us_city_valid.request.code}`)
        .times(1)
    ...
    it('gets information for a place in canada given a city and province', () => {
      nock(URLS.https)
        .get(`/${data.ca_city_valid.request.country}/${data.ca_city_valid.request.code}`)
        .times(1)
    ...
    it('gets information for a place over http (as opposed to https)', () => {
      ...
      wrapper.__set__('API', 'http://api.zippopotamus.us');
      return wrapper
        .query(data.us_code_valid.request)
        .then(response => {
          response.should.deep.equal(data.us_code_valid.response);
          wrapper.__set__('API', 'https://api.zippopotamus.us');
        });
    });
